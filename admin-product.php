...
if (isset($_REQUEST['ok'])) {
    ...
    if (empty($search)) {
        echo'vui long nhap san pham can tim';
    } else {
        // Dùng câu lênh like trong sql và sứ dụng toán tử % của php để tìm kiếm dữ liệu chính xác hơn.
        $query = "select * from tb_product where product_name like '%$search%' or product_id like '%$search%'";
        $total_pages_sql = "SELECT COUNT(*) FROM tb_product where product_name like '%$search%' or product_id like '%$search%'";
        $result = mysqli_query($link, $total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM tb_product where product_name like '%$search%' or product_id like '%$search%' LIMIT $offset, $no_of_records_per_page";
                ...
                <table class="table table-hover table-bordered display"style="max-width: 100%">
                    ...
                    <tbody>
                        ...
                        while ($p = mysqli_fetch_array($res_data)) {
                            ...
                            echo "<td>$p[4]</td>";
                            echo "<td>$p[5]</td>";
                            echo "<td><a type='button' href='edit-product.php?id=$p[0]' class='btn btn-info'><i class='glyphicon glyphicon-pencil' style='color: wheat' ></i></a>";
